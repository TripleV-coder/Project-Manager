  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [projectsRes, tasksRes, sprintsRes, deliverablesRes] = await Promise.all([
        ...
        authFetch(`/api/tasks${projectFilter}`, { signal: AbortSignal.timeout(10000) }),
        authFetch(`/api/sprints${projectFilter}`, { signal: AbortSignal.timeout(10000) }),
        authFetch(`/api/deliverables${projectFilter}`, { signal: AbortSignal.timeout(10000) }),
      ]);
  ...
  const getBarPosition = useCallback(
    (startDate, endDate) => {
      ...
      const endOffset = Math.min(
        totalDays,
        Math.ceil((end - firstDate) / (1000 * 60 * 60 * 24)) + 1
      );
    ...
    scheduledTasks.forEach((task) => {
      if (task.dépendances && task.dépendances.length > 0) {
        task.dépendances.forEach((dep) => {
          ...
          if (dependentTask) {
            const fromPos = getBarPosition(
              dependentTask.date_début || dependentTask.date_échéance,
              dependentTask.date_échéance
            );
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white mb-1">
            Planning & Calendrier Global (Roadmap)
          </h1>
          <p className="text-gray-600 dark:text-gray-400 text-sm lg:text-base">
            Visualisez les dates clés, les jalons et l'enchaînement de vos activités dans le temps.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select value={viewMode} onValueChange={setViewMode}>
            ...
            <SelectContent>
              ...
              <SelectItem value="quarter">Vue par Trimestre</SelectItem>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">Tâches planifiées</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{totalTasks}</p>
                <span className="text-[10px] text-gray-400">Avec dates de début/fin</span>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <span className="text-[10px] text-green-500">Accomplies</span>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">À surveiller</p>
                <p className="text-2xl font-bold text-red-600">{overdueTasks}</p>
                <span className="text-[10px] text-red-500">Date limite dépassée</span>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">Périodes de travail</p>
                <p className="text-2xl font-bold text-purple-600">{sprints.length}</p>
                <span className="text-[10px] text-purple-500">Sprints actifs & passés</span>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-4">
              ...
              <span className="font-medium text-gray-700">
                {dependencyLines.length} dépendance{dependencyLines.length > 1 ? 's' : ''} détectée
                {dependencyLines.length > 1 ? 's' : ''}
              </span>
            ...
            {deliverables.length > 0 && (
              <>
                ...
                {deliverables.map((deliverable) => {
                  const position = deliverable.date_échéance
                    ? getBarPosition(deliverable.date_échéance, deliverable.date_échéance)
                    : null;
                  const statusColor =
                    {
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent>
                                ...
                                {deliverable.date_échéance && (
                                  <p className="text-xs">
                                    Échéance:{' '}
                                    {new Date(deliverable.date_échéance).toLocaleDateString(
                                      'fr-FR'
                                    )}
                                  </p>
                                )}
            ...
            {scheduledTasks.length === 0 ? (
              ...
              scheduledTasks.map((task, taskIndex) => {
                const position = getBarPosition(
                  task.date_début || task.date_échéance,
                  task.date_échéance
                );
                        ...
                        {hasDependencies && showDependencies && (
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger>
                                <Link2
                                  className={`w-3 h-3 ${isBlocked ? 'text-red-500' : 'text-gray-400'}`}
                                />
                              </TooltipTrigger>
                              <TooltipContent>
                                <p className="text-xs">
                                  {task.dépendances.length} dépendance
                                  {task.dépendances.length > 1 ? 's' : ''}
                                </p>
                                {isBlocked && (
                                  <p className="text-xs text-red-500">
                                    Bloquée par des tâches non terminées
                                  </p>
                                )}
                              </TooltipContent>
                            </Tooltip>
                      ...
                      {showDependencies && (blocking.length > 0 || blockedBy.length > 0) && (
                        <div className="flex gap-2 mt-1">
                          {blockedBy.length > 0 && (
                            <Badge
                              variant="outline"
                              className="text-[10px] px-1 py-0 text-red-600 border-red-300"
                            >
                            ...
                            <TooltipContent className="max-w-xs">
                              ...
                              {hasDependencies && (
                                <div className="mt-2 pt-2 border-t">
                                  <span className="text-sm font-medium">Liens de dépendance</span>
                                  {blockedBy.map((t) => (
