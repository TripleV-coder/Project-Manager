...
export default function AuditDashboard() {
    ...
    const loadOptions = async () => {
      try {
        ...
        const [actionsRes, projectsRes] = await Promise.all([
          authFetch('/api/audit/actions'),
          authFetch('/api/projects?limit=200'),
        ]);
        ...
        if (projectsRes.ok) {
          ...
          setAvailableProjects(projectsData.projects || projectsData.data || []);
        }

        // Charger les utilisateurs seulement si permission (adminConfig a déjà été vérifié)
        try {
          const usersRes = await authFetch('/api/users?limit=100');
          if (usersRes.ok) {
  ...
  const loadAuditData = useCallback(async () => {
    try {
      ...
      const [summaryRes, logsRes] = await Promise.all([
        ...
        authFetch(
          `/api/audit?${new URLSearchParams(Object.entries(filters).filter(([_, v]) => v))}`
        ),
      ]);
  ...
  const handleExport = async (format = 'csv') => {
    try {
      ...
      const response = await authFetch(`/api/audit/export?${params}`, {});
  ...
  const activeFiltersCount = Object.entries(filters).filter(
    ([k, v]) => v && !['limit', 'skip'].includes(k)
  ).length;

  const getSeverity = (s) =>
    severityConfig[s] || { label: s, bg: 'bg-gray-100', text: 'text-gray-600', dot: 'bg-gray-400' };
          ...
          <Popover>
            ...
            <PopoverContent className="w-28 p-1">
              <button
                ...
                className="w-full px-2 py-1.5 text-xs text-left hover:bg-gray-100 rounded"
              >
                CSV
              </button>
              <button
          ...
          <div className="flex gap-2 items-center">
            ...
            >
              ...
              {activeFiltersCount > 0 && (
                <span
                  className={`text-[10px] px-1 rounded ${showFilters ? 'bg-white/20' : 'bg-indigo-100 text-indigo-700'}`}
                >
                  {activeFiltersCount}
                </span>
              )}
              ...
              {filters.projectId && (
                <div className="flex items-center gap-2 p-2 bg-indigo-50 rounded-lg">
                  ...
                  <span className="text-xs text-indigo-700">
                    ...
                    <strong>
                      {availableProjects.find((p) => p._id === filters.projectId)?.nom}
                    </strong>
                  </span>
                  <button
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="text-left px-3 py-2 text-xs font-medium text-gray-500 hidden lg:table-cell">
                  Projet
                </th>
                <th className="text-left px-3 py-2 text-xs font-medium text-gray-500 hidden xl:table-cell">
                  Sévérité
                </th>
              ...
              {filteredLogs.length === 0 ? (
                ...
                filteredLogs.map((log, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <span
                        className={`inline-flex px-1.5 py-0.5 text-[10px] font-medium rounded ${getActionStyle(log.action)}`}
                      >
                        {log.action}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-2 hidden xl:table-cell">
                      <span
                        className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium rounded ${getSeverity(log.severity).bg} ${getSeverity(log.severity).text}`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${getSeverity(log.severity).dot}`}
                        ></span>
          ...
          <div className="px-4 py-3 border-t bg-gray-50 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500">
                {filters.skip + 1}-{Math.min(filters.skip + filters.limit, totalResults)} sur{' '}
                {totalResults}
              </span>
              <select
                value={filters.limit}
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <span
                    className={`inline-flex px-2 py-0.5 text-xs font-medium rounded ${getActionStyle(selectedLog.action)}`}
                  >
                    {selectedLog.action}
                  </span>
                </div>
                <div>
                  ...
                  <span
                    className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded ${getSeverity(selectedLog.severity).bg} ${getSeverity(selectedLog.severity).text}`}
                  >
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${getSeverity(selectedLog.severity).dot}`}
                    ></span>
                ...
                <div>
                  ...
                  <p className="text-xs">
                    {selectedLog.navigateur || '-'} / {selectedLog.os || '-'}
                  </p>
                </div>
              </div>
