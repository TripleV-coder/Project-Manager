...
export const RATE_LIMIT_CONFIG = {
  ...
  // Token refresh: every active session refreshes roughly once per access-
  // token lifetime (15 min) on its own, and several users can legitimately
  // share one IP (office NAT) — needs more headroom than the `auth` bucket
  // shared with sensitive one-off actions like first-login-reset.
  refresh: {
    windowMs: 15 * 60 * 1000,
    max: 60,
  },
...
export function getClientIP(request) {
  ...
  // Distinguish "unset" (default to 1 trusted hop) from an EXPLICIT 0, which
  // means "no reverse proxy in front of us at all". Number.parseInt(...) || 1
  // used to collapse both cases to 1, silently trusting XFF for a
  // directly-exposed deployment that explicitly asked not to.
  ...
  if (raw === undefined || raw === '') {
    ...
    trusted = Number.isNaN(parsed) || parsed < 0 ? 1 : parsed;
  }

  if (trusted === 0) {
    // No trusted proxy: XFF and X-Real-IP are entirely client-controlled and
    // must be ignored outright (fail closed) rather than trusted. Fall back
    // to the raw connection address if the runtime exposes one; there is no
    // spoofable-header fallback here by design.
    return request?.socket?.remoteAddress || 'unknown';
  }
