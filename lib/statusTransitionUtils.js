...
export function getNextStatuses(entity, entityType, userPermissions = {}) {
  ...
  const availableStatusIds = getAvailableTransitions(entityType, currentStatus, userPermissions);
  ...
  return {
    ...
    autoTransition: autoTransition
      ? {
          ...
          readyInDays: Math.max(
            0,
            (autoTransition.variationDays ?? autoTransition.originalDays) -
              Math.floor((new Date() - new Date(entity.updated_at)) / (1000 * 60 * 60 * 24))
          ),
        }
      : null,
...
export function getTransitionRequirements(entityType, fromStatus, toStatus) {
  const config = {
    ...
    project: {},
    deliverable: {
      'En validation:Validé': { permissions: ['validerLivrable', 'adminConfig'] },
      'En validation:Refusé': { permissions: ['validerLivrable', 'adminConfig'] },
    },
  };
