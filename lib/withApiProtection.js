...
export function withApiProtection(handler, options = {}) {
  ...
  return async function protectedHandler(request, context) {
    try {
      ...
      const rateLimitResult = applyRateLimit(request, null, rateLimitConfig);
      ...
      if (requireAuth) {
        ...
        // Re-apply rate limit with user context
        const userRateLimit = applyRateLimit(request, user._id?.toString(), rateLimitConfig);
        if (!userRateLimit.allowed) {
          return handleRateLimitError(userRateLimit);
        }
