...
export function withApiProtection(handler, options = {}) {
  ...
  return async function protectedHandler(request, context) {
    try {
      ...
      // unauthenticated ones, before we do any auth work. This is what
      // stops unauthenticated floods from running unchecked (previously
      // rate limiting only ran after auth, so a 401 flood was unlimited).
      //
      // Trade-off: an authenticated request will hit this IP counter a
      // second time in pass 2 below (applyRateLimit already doubles the
      // IP-side max whenever a userId is passed, since it expects exactly
      // one increment per request). To keep a normal authenticated session
      // from being rate-limited purely by this double-count, pass 1 uses
      // its own doubled headroom.
