...
export const WORKFLOW_CONFIG = {
  ...
  task: {
    statuses: {
      Backlog: {
        ...
        description: 'Tâches en attente de planification',
      },
      'À faire': {
        label: 'À faire',
    ...
    autoTransitions: {
      validé: {
        ...
        condition: (expense) => {
          ...
          const daysValidated = Math.floor(
            (new Date() - new Date(expense.date_validation)) / (1000 * 60 * 60 * 24)
          );
    ...
    autoTransitions: {
      'En cours': {
        ...
        condition: (project) => {
          ...
          const completed =
            (project.stats.tâches_terminées || 0) + (project.stats.sprints_terminés || 0);
  ...
  deliverable: {
    statuses: {
      'À produire': {
        label: 'À produire / En attente',
        color: 'bg-gray-100 text-gray-800',
        description: 'Livrable non encore généré',
      },
      'En validation': {
        label: 'En validation',
        color: 'bg-yellow-100 text-yellow-800',
    ...
    transitions: {
      ...
      'En validation': {
        Validé: {
          allowed: true,
          timeRequired: null,
          permissions: ['validerLivrable', 'adminConfig'],
      ...
      'En validation': {
        ...
        condition: (deliverable) => {
          ...
          const daysSinceSubmit = Math.floor(
            (new Date() - new Date(deliverable.date_soumise)) / (1000 * 60 * 60 * 24)
          );
...
export function checkAutoTransition(entityType, currentStatus, entity) {
  ...
  if (autoTransition.variationFactors) {
    adjustedDays = applyVariationFactors(adjustedDays, autoTransition.variationFactors, entity);
  }
...
export function getEscalationAction(entityType, status, statusStartDate) {
  ...
  const daysSinceStatusChange = Math.floor(
    (new Date() - new Date(statusStartDate)) / (1000 * 60 * 60 * 24)
  );
