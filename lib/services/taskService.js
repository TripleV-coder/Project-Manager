...
class TaskService {
  ...
  async createTask(data, userId) {
    ...
    const task = new Task({
      ...
      // Support both date_échéance and date_deadline for backwards compatibility
      date_échéance: data.date_échéance || data.date_deadline,
      // Support both estimation_heures and heures_estimées for backwards compatibility
      estimation_heures: data.estimation_heures || data.heures_estimées,
   ...
   * OPTIMIZED: Utilise l'agrégation MongoDB au lieu de charger toutes les tâches
   * Cela évite les race conditions et améliore les performances
   */
  async updateProjectStats(projectId) {
    await connectDB();

    // Utiliser l'agrégation MongoDB pour calculer les stats en une seule requête
    // Cela garantit la cohérence des données même avec des requêtes concurrentes
    const [aggregationResult] = await Task.aggregate([
      { $match: { projet_id: projectId } },
      {
        $group: {
          ...
          heures_estimées: { $sum: { $ifNull: ['$estimation_heures', 0] } },
          heures_réelles: { $sum: { $ifNull: ['$temps_réel', 0] } }
        }
      }
    ]);

    const stats = aggregationResult ? {
      total_tâches: aggregationResult.total_tâches || 0,
      tâches_terminées: aggregationResult.tâches_terminées || 0,
      heures_estimées: aggregationResult.heures_estimées || 0,
      heures_réelles: aggregationResult.heures_réelles || 0,
      progression: aggregationResult.total_tâches > 0
        ? Math.round((aggregationResult.tâches_terminées / aggregationResult.total_tâches) * 100)
        : 0
    } : {
      total_tâches: 0,
      tâches_terminées: 0,
    ...
    // Utiliser findOneAndUpdate avec opérations atomiques
