...
class ProjectService {
  /**
   * Invalider le cache d'un projet (exposé pour usage externe)
   */
  ...
  async getAccessibleProjects(userId, userRole, limit = 50, skip = 0) {
    ...
    if (!userRole?.permissions?.voirTousProjets && !userRole?.permissions?.adminConfig) {
      query.$or = [
        { chef_projet: userId },
  ...
  /**
   * Récupérer les IDs des projets accessibles par l'utilisateur
   * Utile pour filtrer les sprints, tasks, etc.
   */
  async getAccessibleProjectIds(userId, userRole) {
    await connectDB();

    // Si l'utilisateur peut voir tous les projets, retourner null (pas de filtre)
    if (userRole?.permissions?.voirTousProjets || userRole?.permissions?.adminConfig) {
      return null;
    }
    ...
    const projects = await Project.find({
      archivé: false,
      $or: [{ chef_projet: userId }, { product_owner: userId }, { 'membres.user_id': userId }],
    })
      .select('_id')
      .lean();
  ...
  async updateProject(projectId, data, userId) {
    ...
    // Récupérer l'ancien projet pour comparaison
    ...
    // Logique d'alerte sur la gouvernance
    if (oldProject && project) {
      const governanceChanged =
        (data.comite_pilotage &&
          JSON.stringify(oldProject.comite_pilotage) !== JSON.stringify(data.comite_pilotage)) ||
        (data.comite_technique &&
          JSON.stringify(oldProject.comite_technique) !== JSON.stringify(data.comite_technique)) ||
        (data.structures_partenaires &&
          JSON.stringify(oldProject.structures_partenaires) !==
            JSON.stringify(data.structures_partenaires)) ||
        (data.objectifs && oldProject.objectifs !== data.objectifs) ||
        (data.contexte && oldProject.contexte !== data.contexte) ||
        (data.termes_de_reference && oldProject.termes_de_reference !== data.termes_de_reference);

      if (governanceChanged) {
        await notificationService.notifyGovernanceChange(projectId, userId);
      }
    }
  ...
  async addProjectMember(projectId, userId, projectRoleId) {
    ...
    if (!projectRoleId) {
      throw new Error('Chaque membre doit avoir un rôle projet');
    }
    ...
    const projectRole = await ProjectRole.findOne({
      ...
      .lean();
    if (!projectRole) {
      throw new Error('Le rôle projet ne correspond pas à ce projet');
    }
  ...
  async getProjectStats(projectId) {
    ...
    const project = await Project.findById(projectId).select('stats').lean();
