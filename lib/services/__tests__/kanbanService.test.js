...
class KanbanService {
  async getKanbanBoard(projectId) {
    ...
    const project = await Project.findById(projectId).select('colonnes_kanban').lean();
  ...
  async getTasksByColumn(projectId, column) {
    ...
    return Task.find({
      projet_id: projectId,
      $or: [{ colonne_kanban: column }, { statut: column }],
    })
      .populate('assigné_à', 'nom_complet')
  ...
  async reorderTasks(projectId, column, taskIds) {
    ...
    const updates = taskIds.map((taskId, index) =>
      Task.findByIdAndUpdate(taskId, { ordre_priorité: index }, { new: true })
    );
...
describe('KanbanService', () => {
  ...
  describe('moveTask', () => {
    ...
    it('should throw error if task not found', async () => {
      ...
      await expect(kanbanService.moveTask('nonexistent', 'En cours')).rejects.toThrow(
        'Tâche non trouvée'
      );
    });
  ...
  describe('getTasksByColumn', () => {
    ...
    it('should handle tasks with matching colonne_kanban or statut', async () => {
      ...
      expect(Task.find).toHaveBeenCalledWith(
        expect.objectContaining({
          $or: expect.arrayContaining([{ colonne_kanban: 'En cours' }, { statut: 'En cours' }]),
        })
      );
    });
  });
      ...
      const mockTasks = [
        {
          ...
          story_points: 5,
          labels: ['blocked'],
          date_échéance: new Date(Date.now() + 86400000),
        },
        {
          _id: 'task-2',
          story_points: 3,
