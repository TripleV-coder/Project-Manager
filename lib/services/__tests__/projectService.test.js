...
describe('ProjectService', () => {
  ...
  describe('getAccessibleProjects', () => {
    it('should return all projects if user is admin', async () => {
      ...
      const result = await projectService.getAccessibleProjects(userId, {
    ...
    it('should filter projects by user access if not admin', async () => {
      ...
      const result = await projectService.getAccessibleProjects(userId, { permissions: {} }, 50, 0);
  ...
  describe('getProjectById', () => {
    it('should fetch and return project with full details', async () => {
      ...
      const _result = await projectService.getProjectById(projectId);
    ...
    it('should return true if user is project member', async () => {
      ...
      Project.findById.mockReturnValue({
        ...
        lean: jest.fn().mockResolvedValue({
          ...
          membres: [{ user_id: userId, project_role_id: 'role-1' }],
        }),
      });
  ...
  describe('getProjectStats', () => {
    it('should calculate project statistics correctly', async () => {
      const projectId = 'proj-123';
      // Using correct field names from Task model:
      // - statut 'Terminé' (not 'Terminée')
      // - estimation_heures (not heures_estimées)
      // - temps_réel (not heures_réelles)
      const mockTasks = [
        { _id: 'task-1', statut: 'Terminé', estimation_heures: 5, temps_réel: 6 },
        { _id: 'task-2', statut: 'En cours', estimation_heures: 8, temps_réel: 0 },
        { _id: 'task-3', statut: 'À faire', estimation_heures: 3, temps_réel: 0 },
      ];
  ...
  describe('addProjectMember', () => {
    it('should add member to project', async () => {
      ...
      ProjectRole.findOne.mockReturnValue({
        select: jest.fn().mockReturnThis(),
        lean: jest.fn().mockResolvedValue({ _id: roleId }),
      });
    ...
    it('rejects a member without a project role of that project', async () => {
      await expect(projectService.addProjectMember('proj-123', 'user-456', null)).rejects.toThrow(
        'Chaque membre doit avoir un rôle projet'
      );
      ...
      await expect(
        projectService.addProjectMember('proj-123', 'user-456', 'role-foreign')
      ).rejects.toThrow('Le rôle projet ne correspond pas à ce projet');
