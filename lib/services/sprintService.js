...
class SprintService {
  async getProjectSprints(projectId, limit = 50, skip = 0) {
    ...
    const [sprints, total] = await Promise.all([
      Sprint.find({ projet_id: projectId }).sort({ date_début: -1 }).skip(skip).limit(limit).lean(),
  ...
  async getSprintById(sprintId) {
    await connectDB();
    return Sprint.findById(sprintId).populate('projet_id', 'nom').lean();
  }
  ...
  async completeSprint(sprintId) {
    ...
    await Task.updateMany({ sprint_id: sprintId, statut: { $ne: 'Terminé' } }, { sprint_id: null });
