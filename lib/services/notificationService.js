...
class NotificationService {
  ...
  async createNotification({
    ...
    try {
      ...
      log.error('[NotificationService] Erreur création notification:', error);
      return null;
    }
  }
  ...
  /**
   * La personne destinataire d'une tâche doit pouvoir la voir chez elle.
   */
  async notifyTaskAssigned(task, assigneeId, assignerId) {
    if (!assigneeId || (assignerId && String(assigneeId) === String(assignerId))) return;

    const titre = task.titre || 'une tâche';
    await this.createNotification({
      destinataire: assigneeId,
      type: 'assignation_tâche',
      titre: 'Tâche assignée',
      message: `« ${titre} » vous a été assignée.`,
      entity_type: 'tâche',
      entity_id: task._id,
      entity_nom: titre,
      lien: `/dashboard/tasks`,
      expéditeur: assignerId,
      priority: 'info',
    });
  }
