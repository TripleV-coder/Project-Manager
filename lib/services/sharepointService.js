...
const log = createLogger('sharepoint-service');
...
export const loadConfigFromDB = async (forceRefresh = false) => {
  ...
  if (
    ...
    cachedConfig &&
    configLastFetch &&
    Date.now() - configLastFetch < CONFIG_CACHE_TTL
  ) {
    return cachedConfig;
  }
  ...
  try {
    ...
    log.error('Failed to load SharePoint config from DB:', error);
    return null;
  }
};
...
export const getAccessToken = async (customConfig = null) => {
  ...
  try {
    ...
    log.error('Failed to acquire token:', error);
...
const graphRequest = async (endpoint, options = {}, customConfig = null) => {
  ...
  const _config = customConfig || (await getConfig());
...
export const createProjectFolder = async (projectName, projectId) => {
  ...
  const folderName = `Projet_${projectId}_${projectName
...
export const getOrCreateProjectFolder = async (projectId, projectName) => {
  try {
    ...
    log.error('Error creating project folder:', error);
    throw error;
  }
};
  ...
  try {
    ...
    for (const file of localFiles) {
      ...
      try {
        ...
        const content = file.url || file.content;

        const uploaded = await uploadFile(
          folder.id,
          normalizedName,
          content,
          file.type_mime || file.mimeType
        );
  ...
  try {
    ...
    for (const project of projects) {
      try {
        ...
        const syncResult = await syncProjectFiles(project._id.toString(), project.nom, files);
