...
class TimesheetService {
  ...
  async submitTimesheet(timesheetId) {
    ...
    return Timesheet.findByIdAndUpdate(timesheetId, { statut: 'soumis' }, { new: true }).lean();
  }
  ...
  async validateTimesheet(timesheetId, validatedBy, approved = true, comment = '') {
    ...
    if (approved && timesheet.task_id) {
      await Task.findByIdAndUpdate(timesheet.task_id, { $inc: { temps_réel: timesheet.heures } });
    }

    // Invalider le cache du projet pour refléter les changements
    if (timesheet.projet_id && projectService?.invalidateCache) {
      projectService.invalidateCache(timesheet.projet_id.toString());
    }
  ...
  async getProjectTimesheetStats(projectId, dateStart, dateEnd) {
    ...
    return {
      total_hours: timesheets.reduce((sum, t) => sum + (t.heures || 0), 0),
      validated_hours: validated.reduce((sum, t) => sum + (t.heures || 0), 0),
      billable_hours: validated
        .filter((t) => t.facturable)
        .reduce((sum, t) => sum + (t.heures || 0), 0),
