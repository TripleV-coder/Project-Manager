...
export const initEmailTransporter = () => {
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    log.warn('[WARNING] Email service: SMTP credentials not configured');
    return null;
  }

  try {
    transporter = nodemailer.createTransport(defaultConfig);
    log.info('[OK] Email transporter initialized');
    return transporter;
  } catch (error) {
...
export const sendEmail = async ({ to, subject, text, html }) => {
  ...
  try {
    ...
    log.info('[OK] Email sent:', { messageId: info.messageId });
