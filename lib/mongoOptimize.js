...
export const PROJECTIONS = {
  user: {
    normal:
      'nom_complet email avatar poste_titre département_équipe status role_id dernière_connexion created_at updated_at',
  ...
  project: {
    normal:
      'nom description statut priorité date_début date_fin_prévue date_fin_réelle stats archivé chef_projet product_owner membres template_id created_at updated_at',
  ...
  task: {
    normal:
      'titre description statut priorité type assigné_à projet_id sprint_id deliverable_id date_échéance date_début estimation_heures temps_réel story_points ordre_priorité colonne_kanban labels tags checklist créé_par created_at updated_at',
    ...
    kanban:
      'titre description statut priorité type assigné_à ordre_priorité colonne_kanban story_points labels',
  },
  sprint: {
    normal:
      'nom objectif statut projet_id date_début date_fin capacité_équipe story_points_planifiés story_points_complétés velocity created_at',
  ...
  file: {
    normal:
      'nom type_mime taille dossier projet_id entity_type entity_id uploadé_par tags description created_at',
    list: 'nom type_mime taille dossier projet_id uploadé_par created_at',
    minimal: 'nom type_mime taille',
  },
  comment: {
    normal:
      'contenu contenu_html entity_type entity_id auteur parent_id niveau thread_id mentions fichiers_joints reactions édité résolu created_at updated_at',
    list: 'contenu entity_type entity_id auteur parent_id created_at',
    minimal: 'contenu auteur created_at',
  },
  timesheet: {
    normal:
      'utilisateur projet_id task_id sprint_id date heures description type_saisie statut facturable taux_horaire montant created_at',
