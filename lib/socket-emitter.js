...
const log = createLogger('socket-emitter');

const SOCKET_SERVER_URL = process.env.SOCKET_SERVER_URL || 'http://localhost:4000';
const SOCKET_TIMEOUT_MS = 5000;
const SOCKET_EMIT_SECRET = process.env.SOCKET_EMIT_SECRET || process.env.JWT_SECRET;

function getSocketEmitterHeaders() {
  const headers = {
    'Content-Type': 'application/json',
  };

  if (SOCKET_EMIT_SECRET) {
    headers['x-socket-secret'] = SOCKET_EMIT_SECRET;
  }
...
function sendSocketEvent(payload) {
  ...
  fetch(`${SOCKET_SERVER_URL}/emit`, {
    method: 'POST',
    headers: getSocketEmitterHeaders(),
    body: JSON.stringify(payload),
    signal: controller.signal,
  })
    .catch((error) => {
      if (process.env.NODE_ENV === 'development') {
        log.debug('Socket event delivery failed (non-critical):', { err: error });
      }
    })
    .finally(() => {
...
export async function disconnectUserSockets(userId, reason = 'revoked') {
  sendSocketEvent({
