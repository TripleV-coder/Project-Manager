...
const log = createLogger('audit-api-handler');
...
export async function handleGetAuditSummary(url, user) {
  try {
    ...
    return NextResponse.json({
      ...
      activitiesByAction: Object.fromEntries(activitiesByAction.map((a) => [a._id, a.count])),
      recentCriticalActivities,
    });
  } catch (error) {
    log.error('Error getting audit summary:', error);
    return NextResponse.json(
...
export function handleGetAvailableActions(user) {
  try {
    ...
    log.error('Error getting available actions:', error);
    return NextResponse.json({ error: 'Failed to get available actions' }, { status: 500 });
  }
}
