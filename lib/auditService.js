...
function parseUserAgent(userAgent) {
  try {
    ...
    log.error('Error parsing user agent:', error);
    return {
      navigateur: 'Unknown',
...
async function getGeoLocation(ipAddress) {
  try {
    ...
    log.error('Error getting geolocation:', error);
    return { country: null, city: null };
  }
}
...
export async function createUserSession(user, sessionToken, request) {
  try {
    ...
    log.error('Error creating user session:', error);
    return null;
  }
}
...
export async function endUserSession(sessionToken) {
  try {
    ...
    log.error('Error ending user session:', error);
  }
}
...
export async function getUserActivityHistory(userId, filters = {}) {
  try {
    ...
    log.error('Error getting user activity history:', error);
    throw error;
  }
}
...
export async function getAuditLogs(filters = {}) {
  try {
    ...
    if (entityType) query.entity_type = entityType;
    if (severity) query.severity = severity;
    if (projectId) query.related_project_id = projectId;
    ...
    const logs = await AuditLog.find(query)
      ...
      .populate('related_project_id', 'nom')
      .lean();
...
export async function getAuditStatistics(options = {}) {
  try {
    ...
    log.error('Error getting audit statistics:', error);
    throw error;
  }
}
