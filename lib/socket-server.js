...
export function initializeSocket(httpServer) {
  ...
  io.on('connection', (socket) => {
    log.info(`User connected: ${socket.userEmail} (${socket.userId})`);
    ...
    socket.on('join:project', async (projectId) => {
      try {
        ...
        log.info(`User ${socket.userEmail} joined project ${projectId}`);
      } catch (error) {
    ...
    socket.on('leave:project', (projectId) => {
      ...
      log.info(`User ${socket.userEmail} left project ${projectId}`);
    });

    // Disconnect event
    socket.on('disconnect', () => {
      log.info(`User disconnected: ${socket.userEmail}`);
    });
  });
...
export async function emitToProjectMembers(projectId, event, data, requiredPermission = null) {
  ...
  for (const member of members) {
    ...
    const memberSocket = io.sockets.sockets.get(
      Array.from(io.sockets.sockets.values()).find((s) => s.userId === userId)?.id
    );

    if (
      memberSocket &&
      (!requiredPermission || memberSocket.userPermissions?.[requiredPermission])
    ) {
