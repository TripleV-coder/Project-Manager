...
export async function checkRateLimitByIP(request, namespace, config) {
  // Use the trust-boundary-hardened resolver from lib/rateLimit.js — it only
  // trusts the rightmost TRUSTED_PROXY_COUNT hops of X-Forwarded-For in
  // production. Re-deriving the IP here from the raw leftmost XFF entry (as
  // this used to do) would let an attacker spoof a fresh bucket on every
  // request whenever REDIS_URL is configured.
