...
describe('Advanced Menu Configuration Tests', () => {
  describe('Menu Items Structure', () => {
    ...
    it('should have correct structure for notifications menu', () => {
      ...
      expect(NOTIFICATIONS_MENU).toHaveProperty('labelKey');
  ...
  describe('Menu Permission Mapping', () => {
    it('should map correct permissions to main menu items', () => {
      const expectedMappings = {
        ...
        '/dashboard/tasks': 'deplacerTaches',
  ...
  describe('filterMenuItemsByPermissions', () => {
    ...
    it('shows Mes tâches when the user can move cards even without gererTaches', () => {
      const member = {
        _id: 'member-1',
        role: {
          permissions: {
      ...
      const hrefs = filterMenuItemsByPermissions(MAIN_MENU_ITEMS, member).map((item) => item.href);
  ...
  describe('Role-Specific Menu Visibility', () => {
    ...
    testCases.forEach(
      ({ name, expectedMainMenus, expectedAdminMenus, permissions, visibleMenus }) => {
  ...
  describe('Admin Menu Items Count', () => {
    it('should have expected admin menu items', () => {
      const adminLabels = ADMIN_MENU_ITEMS.map((item) => item.labelKey);

      expect(adminLabels).toContain('rolesPermissions');
      expect(adminLabels).toContain('users');
      expect(adminLabels).toContain('projectTemplates');
      expect(adminLabels).toContain('deliverableTypes');
      expect(adminLabels).toContain('sharepoint');
      expect(adminLabels).toContain('auditLogs');
  ...
  describe('Main Menu Items Count', () => {
    it('should have expected main menu items', () => {
      const mainLabels = MAIN_MENU_ITEMS.map((item) => item.labelKey);

      expect(mainLabels).toContain('dashboard');
      expect(mainLabels).toContain('projects');
      ...
      expect(mainLabels).toContain('tasks');
      expect(mainLabels).toContain('files');
      expect(mainLabels).toContain('comments');
      expect(mainLabels).toContain('timesheets');
      expect(mainLabels).toContain('budget');
      expect(mainLabels).toContain('reports');
