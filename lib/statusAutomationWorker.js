import { evaluateAutoTransition, evaluateEscalation } from './statusTransitionUtils';
...
const log = createLogger('status-automation-worker');
...
export async function processAutoTransitionsForEntity(entityType) {
  try {
    ...
    for (const entity of entities) {
      ...
      if (autoTransition) {
        ...
        try {
          ...
          log.error(`Error auto-transitioning ${entityType} ${entity._id}:`, error);
        }
      }
    ...
    log.error(`Error processing auto-transitions for ${entityType}:`, error);
    return {
      success: false,
      error: error.message,
      entityType,
...
export async function processAllAutoTransitions() {
  ...
  for (const entityType of Object.keys(ENTITY_CONFIGS)) {
    try {
      ...
      log.error(`Error processing ${entityType}:`, error);
  ...
  try {
    ...
    switch (escalation.action) {
      ...
      case 'process_payment':
        ...
        await ENTITY_CONFIGS[entityType].model.findByIdAndUpdate(entity._id, {
          statut: 'payé',
    ...
    log.error('Error handling escalation:', error);
  }
}
...
export async function checkOverdueTasks() {
  try {
    ...
    log.error('Error checking overdue tasks:', error);
    return {
      success: false,
      error: error.message,
    };
