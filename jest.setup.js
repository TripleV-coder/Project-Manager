...
// Polyfill pour Request/Response web API requis par next/server
...
jest.mock('mongoose', () => {
  ...
  const model = function (name, _schema) {
    if (!models[name]) {
      ...
      mockModel.findOneAndUpdate = jest.fn(function () {
        return createQueryHelper(null);
      });
  ...
  if (mongoose && mongoose.models) {
    Object.keys(mongoose.models).forEach((modelName) => {
      ...
      if (model && typeof model === 'function') {
        ...
        model.findOneAndUpdate.mockImplementation(function () {
          return createQueryHelper(null);
        });
