...
const testSuites = [
  { name: 'RBAC Permissions', pattern: 'permissions', file: 'lib/__tests__/permissions.test.js' },
  {
    ...
    pattern: 'menuVisibility',
    file: 'lib/__tests__/menuVisibility.test.js',
  },
  {
    name: 'useRBACPermissions Hook',
    pattern: 'useRBACPermissions',
    file: 'hooks/__tests__/useRBACPermissions.test.js',
  },
  {
    name: 'Project Service',
    pattern: 'projectService',
    file: 'lib/services/__tests__/projectService.test.js',
  },
  {
    name: 'Task Service',
    pattern: 'taskService',
    file: 'lib/services/__tests__/taskService.test.js',
  },
  {
    name: 'User Service',
    pattern: 'userService',
    file: 'lib/services/__tests__/userService.test.js',
  },
  {
    name: 'Sprint Service',
    pattern: 'sprintService',
    file: 'lib/services/__tests__/sprintService.test.js',
  },
  {
    name: 'Kanban Service',
    pattern: 'kanbanService',
    file: 'lib/services/__tests__/kanbanService.test.js',
  },
  {
    name: 'Timesheet Service',
    pattern: 'timesheetService',
    file: 'lib/services/__tests__/timesheetService.test.js',
  },
  {
    name: 'Budget Service',
    pattern: 'budgetService',
    file: 'lib/services/__tests__/budgetService.test.js',
  },
];
...
function displayResults(results) {
  ...
  const passPercentage =
    results.totalTests > 0 ? ((results.passedTests / results.totalTests) * 100).toFixed(1) : 0;
  ...
  log(
    `Failed:         ${results.failedTests > 0 ? colors.red : colors.green}${results.failedTests > 0 ? '✗' : '✓'} ${results.failedTests}${colors.reset}`
  );
  ...
  Object.entries(categories).forEach(([category, patterns]) => {
    const categoryResults = patterns
      .map((pattern) => {
        const suite = testSuites.find((s) => s.pattern === pattern);
        return suite;
      })
  ...
  testSuites.forEach((suite, index) => {
    ...
    log(
      `  ${index + 1}. ${statusIcon}${colors.reset} ${suite.name}${colors.dim} (${suite.file})${colors.reset}`
    );
  });
  ...
  log(
    `${colors.green}✓ RBAC System${colors.reset}           - 65+ tests covering 23 permissions, 14 menus`
  );
  log(
    `${colors.green}✓ Core Services${colors.reset}        - 115+ tests for Projects, Tasks, Users`
  );
  log(
    `${colors.green}✓ Agile Modules${colors.reset}        - 175+ tests for Sprints, Kanban, Timesheet, Budget`
  );
  log(
    `${colors.green}✓ Menu Visibility${colors.reset}      - 45+ tests for role-based menu access`
  );
  ...
  logSection(
    `${results.failedTests === 0 ? '✓' : '✗'} TEST RUN ${results.failedTests === 0 ? 'SUCCESSFUL' : 'INCOMPLETE'}`
  );
